...
if(FALSE){
  FORD <- getSymbols("F", from="1970-01-01", to = "2019-01-01", auto.assign = FALSE)
  getSymbols("GM", from="1970-01-01", to = "2019-01-01")
  
  FORD <- FORD %>% 
    tsbox::ts_tbl() %>% 
    pivot_wider(id_cols = time, names_from = "id", values_from = "value") %>% 
    dplyr::select(time, F.Close)
  
  GM <- GM %>% 
    tsbox::ts_tbl() %>% 
    pivot_wider(id_cols = time, names_from = "id", values_from = "value") %>% 
    dplyr::select(time, GM.Close)
  
  inner_join(FORD, GM, by = c("time"="time")) %>% 
    arrange(desc(time)) %>% 
    write_csv(file = "./database/Lista4_ex2.csv")
}

tbl <- read_csv(file = "./database/Lista4_ex2.csv")

# oredena corretamente a base de dados
tbl <- tbl %>% arrange(time)

tbl$ret_KO <- log(tbl$F.Close/dplyr::lag(tbl$F.Close))
tbl$ret_PEP <- log(tbl$GM.Close/dplyr::lag(tbl$GM.Close))
